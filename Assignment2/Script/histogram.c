...
// Will be defined at prerocessor step (see makefile)
...
void run_test(int num_elements) 
{
  ...
  printf("___________ Thread Count: %d, Input_data size:%d ______________\n", NUM_THREADS, num_elements);
...
void compute_using_openmp(int *input_data, int *histogram, int num_elements, int histogram_size)
{
  ...
  omp_set_num_threads(NUM_THREADS);
	...
	// Initialize histogram
  #pragma omp parallel for default(none) private(i) shared(histogram, histogram_size)
  for(i = 0; i < histogram_size; i++) 
			 histogram[i] = 0;
  ...
  // and partial sum is calculated for these chunks of data. 
  #pragma omp parallel private(i) shared(partial, input_data)
  {
    ...
    // This is where the data is merged back into the main histogram pointer. 
