...
int i, x, t;
	struct timeval start, stop;	
  	...
  	// Initialize histogram (done serially since histogram_size is small)
  	for(i = 0; i < histogram_size; i++) 
  	{
  		histogram[i] = 0; 	
  	}
	...
	// Bin the elements in the input stream (done parallely)
	# pragma omp parallel num_threads(NUM_THREADS)
	{
		int num_threads = omp_get_num_threads();
		int my_rank = omp_get_thread_num();

		// array of local copies of histogram for each threads
		int** local_histogram = (int**)malloc(sizeof(int*) * num_threads);

		int bucket;
		
		// allocate memory for the array of all histogram copies
		for(i=0; i < num_threads;i++)
		{
			local_histogram[i] = (int*)malloc(sizeof(int) * histogram_size);
		}
		
		// each thread can work on its own local copy of histogram independently
		# pragma omp for nowait
		for(i=0; i < num_elements; i++)
		{
			bucket = input_data[i];
			local_histogram[my_rank][bucket]++;
		}

		// now reduce all local histogram to the global histogram by addition
		for(i=0; i < histogram_size; i++)
		{
		    // following operation need to be mutually exclusive to access shared data
            	    # pragma omp atomic
		    histogram[i] += local_histogram[my_rank][i];
		}
